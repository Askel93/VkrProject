...
public class OwnOperatorController extends BaseController<OwnOperator, String> {
	...
	public ResponseEntity<?> getById(@PathVariable("id") String name) throws ResourceNotFoundException {
		return ResponseEntity.ok().body(OwnOperatorResponse.toResponse(service.findById(name)));
	}

	@GetMapping("")
	@JsonView(View.UI.class)
	public ResponseEntity<?> getPageWithSort(@RequestParam(value = "page", required = false, defaultValue = "0") int page,
